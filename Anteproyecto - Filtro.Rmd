...
```{r}
sprintf("Coeficiente de Correlación: %f", cor(dsuaz$promedio, dsuaz$hrs_actsocial_dia, use="pairwise.complete.obs"))
```

#### In this correlation given, shows a high relation between the difficulty of career and the passion of career.
```{r}
sprintf("Coeficiente de Correlación: %f", cor(dsuaz$dificultad_licenciatura, dsuaz$pasion_licenciatura, use="pairwise.complete.obs"))
```

#### In this correlation we cofirm that there is a few relation within the data, almost is 0
```{r}
sprintf("Coeficiente de Correlación: %f", cor(dsuaz$hrs_estudio_dia, dsuaz$pasion_licenciatura, use="pairwise.complete.obs"))
```

#### In this correlation we cofirm that there is a few relation within the data, almost is 0
```{r}
sprintf("Coeficiente de Correlación: %f", cor(dsuaz$hrs_estudio_dia, dsuaz$promedio, use="pairwise.complete.obs"))
```
