...
### Compute Distances
###### First, we genrate the vectors using rnorm(). Each vector should have the same quantity of observations but different mean and SD. Store the vectors in a data.frame named distances where variables vector01 and vector02
correspond to the two vectors previously generated.
```{r}
a = dsuaz[,c(3)]
b = dsuaz[,c(5)]
...
Add 3 columns to the data.frame.
 The first column should store the euclidean distance between vector01 and vector02.
â€¢ The second column should store the Manhattan distance between vector01 and vector02.
â€¢ The third column should store the Canberra distance between vector01 and vector02.
â€¢ The fourth column should store the Mahalabobis distance between vector01 and vector02.
