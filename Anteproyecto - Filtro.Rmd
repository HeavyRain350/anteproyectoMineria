...
###### In this histogram, almost 72% of the students study 2 to 4 hours every day
###### Skewness equals -0.428, which means that the distribution of the data is approximately symmetric. In addition we can observe that the values of mean and mode are 4.248 and 4.
...
###### In the following case, we have a skewness value of -0.0344, which means that student academic averages are approximately symmetric.
...
###### Now if we draw the distribution's shape we can see that the data is moderately skewed left, because the left tail is longer and the most of the distribution is at the right.
...
###### In this histogrm, we can see that over the 65% of the students have a pasion of 9 and 10 for his/her career.
###### The value of skewness is -1.3513, that means that the distribution of data is highly skewed.
###### If we draw the distribution's shape, we will see that the data is highly skewed left, this can be verified if we look at the results of the summary, there we can realize that the median is greater than the mean, which indicates that in the graph through the distribution's shape we will first find the mean, then the median and finally the fashion At the highest point.
...
###### In this histogram, the most students have difficulty to get a job of 5, 8 and 10.
###### The value of skewness is -0.7729, which means that the distribution of our data is moderately skewed left.
###### As in the previous data set we can see that the median value is higher than the mean.
...
###### Por el valor de skewness, 0.392 podemos decir que la distribuci??n de los datos is approximately symmetric.
###### But if we do the distribution's shape, we can see that the data is tilted to the left, indicating that we have a skewed right or that they are in a positive direction.
...
```{r}
sprintf("Coeficiente de Correlaci?n: %f", cor(dsuaz$promedio, dsuaz$hrs_actsocial_dia, use="pairwise.complete.obs"))
```
...
```{r}
sprintf("Coeficiente de Correlaci?n: %f", cor(dsuaz$dificultad_licenciatura, dsuaz$pasion_licenciatura, use="pairwise.complete.obs"))
```
...
```{r}
sprintf("Coeficiente de Correlaci?n: %f", cor(dsuaz$hrs_estudio_dia, dsuaz$pasion_licenciatura, use="pairwise.complete.obs"))
```

#### Correlation between hours of study per day and GPA.
###### In this correlation we cofirm that there is a few relation within the data, almost is 0
```{r}
sprintf("Coeficiente de Correlaci?n: %f", cor(dsuaz$hrs_estudio_dia, dsuaz$promedio, use="pairwise.complete.obs"))
```
